...
import http from '@/utils/http'
    export default {
        ...
        methods:{
            ...
            async insertprescribe(){
                let that=this
                this.sumprice()
                let checkresult=that.checkouts(that.form.outs)
                if(!checkresult){
                    return
                }
                ...
                //保存处方
                //检验药品信息填写完整
                let str=JSON.stringify(that.form.outs)
                let data={}
                let patientinfo=JSON.parse(localStorage.getItem('userxinxi'));
                data.outss=str//药品信息
                data.card_no=patientinfo.cardNo//就诊人卡号
                data.clinicno=patientinfo.clinicno//门诊号
                data.dept_code=patientinfo.deptcode//
                data.doctor_code=patientinfo.doctor;
                data.visitdate = patientinfo.visitdate;
                data.visitno=patientinfo.visitno;
                data.itemclass=that.form.itemclass//项目类别？
                data.orderclass=that.form.orderclass//医嘱类别
                let url='docDrug/openDrugPres';
                var res = await http.post(that,url,data);
                if(res.errcode==0){
                    that.$Message.info('开方成功')
                    that.initdata()
                    that.init()
                }else{
            ...
            async getfirmid(itemcode,itemspec){
                let that=this
                //获取公司标识
                let url='docDrug/querySpecification';
                var data = {
                    itemcode,itemspec
                }
                var res = await http.post(that,url,data);
            ...
            async adddrug(data){
                ...
                const firmIdresult= await this.getfirmid(itemcode,itemspec)
            ...
            async fetchfrequency(){
                let that=this
                let url='docDrug/queryFrequency'
                var data = {

                }
                var res = await http.post(that,url,data);
                if(res.errcode==0&&res.data!=null){
                     that.dealfetchfrequency(res.data);
                }
            },
            //处理获取的用药途径数据
            ...
            async fetchchannel(){
                let that=this
                let url='docDrug/queryChannel'
                var data = {

                }
                var res = await http.post(that,url,data);
                if(res.errcode==0&&res.data!=null){
                    that.dealfetchchannel(res.data)
                }
            },
            //处理搜索药品搜索到的数据
            dealsearchDrug(){
            ...
            async searchDrug(){
                let that=this
                let url='docDrug/queryChineseDrug'
                let _arr
                let data = {
                ...
                if(res.errcode==0&&res.data!=null){
                    ...
                    for(var i=0;i<res.data.length;i++){
                        if(res.data[i].inventory!=='0'){
                            arr.push(res.data[i])
                        }
                    }
                    this.searchdrugs=arr;
                }else{
            ...
            async dealsearchDrugses(data){
                ...
                var url  ='docDrug/queryChineseMedicine';
                var list = that.searchdrugs;
                var frequency= data.pccname;
                that.infomsg
                 let patientinfo=JSON.parse(localStorage.getItem('userxinxi'));
                 var order_no= patientinfo.order_no//订单号
                var obj = {
                    procjectid,order_no
                }
                var res = await http.post(that,url,obj);
                if(res.errcode==0&&res.data!=null&&res.data!=''){
                    ...
                    for(var i = 0;i<res.data.length;i++){
                        for(let j=0;j<list.length;j++){
                            ...
                            if(res.data[i].cdrug_code==list[j].ypcode){
                                let item=list[j]
                                item.dosAge=res.data[i].dosage;
                                item.frequency=frequency;
                                 item.firmId = res.data[i].firm_id;
                                arr.push(item);
                            }
                        }
                    }
            ...
            async searchCombination(){
                let that=this;
                let url='docDrug/queryMakeUpPrescribe'
                let patientinfo=JSON.parse(localStorage.getItem('userxinxi'));
                var dept_code = patientinfo.deptcode;
                var doctor_code = patientinfo.doctor;
                var data ={
                     dept_code,doctor_code
                }
                var res = await http.post(that,url,data);
                if(res.errcode==0&&res.data!=null){
                    that.modal11=true;
                    that.searchdrugses=res.data;
                }
            },
            //获取患者信息
            getpatientinfo(){
                ...
                this.form.cardno=patientinfo.cardNo//卡号
                // this.form.orderby=data.registerdept//科室编码
                this.form.depname=patientinfo.hosdepname//科室名字
                this.form.docname=patientinfo.hosdocname//医生名字
                // this.form.doccode=patientinfo.doctor//医生编码
                this.form.tradeno=patientinfo.order_no//订单号
