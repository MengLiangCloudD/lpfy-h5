...
import http from '@/utils/http'
    export default {
        ...
        methods:{
            ...
            //搜索药品
            async searchDrug(orderclass){
                let that=this
                let url=''
                if(orderclass=='A'){
                    url='docDrug/queryWesternDrug'
                }else{
                    url='docDrug/queryChineseDrug'
                }
                var data = {
            ...
            async fetchfrequency(){
                ...
                if(res.errcode==0&&res.data!=null){
                     that.dealfetchfrequency(data.data);
                }
            },
            //编辑当前药品
            ...
            async fetchfirmid(itemspec,itemcode){
                //获取公司标识
                let _data
                let that=this
            ...
            adddrug(data){
                ...
                setTimeout( async function(){
                    ...
                    //     itemcode,itemspec
                    // }
                    // var res = await http.post(_this,url,data);
                    // if(res.errcode==0&&res.data!=null){
                    //    drugitem.firmId=res.data[0].firm_id
                    //    drugitem.drugSpec=res.data[0].drug_spec + res.data[0].drug_supplier_id//药品规格（显示）
                    // }
                    var  fetchfirmiddata= await _this.fetchfirmid(itemspec,itemcode)
                    drugitem.firmId=fetchfirmiddata[0].firm_id
                    drugitem.drugSpec=fetchfirmiddata[0].drug_spec+fetchfirmiddata[0].drug_supplier_id//药品规格（显示）
                    //处理药品价格为.01这种格式的数据
