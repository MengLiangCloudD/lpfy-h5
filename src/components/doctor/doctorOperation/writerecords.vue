...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            showinput(value,tip){
                // var ele = document.getElementsByClassName('writecheck-page')[0];
            //    ele.scrollTop
            ...
            async editinsert(data){
                let that=this
                if(data.diagdesc==""){
                    this.modal1=true
                    return
                }
                let url='docMedRec/updateMedicalrecord'
                let patientinfo=JSON.parse(localStorage.getItem('userxinxi'));
                var data1 = {
                        card_no: patientinfo.cardNo,//就诊时间
                        ...
                        order_no:patientinfo.order_no
                }
                var res  =  await http.post(that,url,data1);
                if(res.errcode==0){
                     that.$Message.info('修改成功');
                }else{
                    that.questmodel=true
                    that.infomsg="修改失败！"
                }
            },
            async insert(data){
                let that=this
                let url='docMedRec/openMedicalrecord'
                if(data.diagdesc==""){
                    this.modal1=true
                    return
                }
                ...
                var res  =  await http.post(that,url,data1);
                if(res.errcode==0){
                     that.$Message.info('病历提交成功');
                    localStorage.setItem('ysrecordstate',5)
                    that.ysrecordstate=5
                }else{
                    that.questmodel=true
                    that.infomsg="病历提交失败！"
                }
            ...
            async getcurrentdayrecord(){
                let that = this;
                let patientinfo=JSON.parse(localStorage.getItem('userxinxi'));
                var visitdate = patientinfo.visitdate;
                var card_no = patientinfo.cardNo;
                var doctor_code = patientinfo.doctor;
                var visitno = patientinfo.visitno;
                var data = {
                    visitno,doctor_code,card_no,visitdate
                }
                var url ="docMedRec/queryTheSameDayMedical";
                var res  =  await http.post(that,url,data);
                if(res.errcode==0&&res.data!=null&&res.data!=''){
                    that.ysrecordstate=5
                    localStorage.setItem('ysrecordstate',5)
                    let recordinfo=res.data[0]
                    for(var item in recordinfo){
                        for(var foritem in that.form){
                            if(item==foritem){
                                that.form[foritem]=recordinfo[item]
                            }else if(item=='maternitl'){
                                that.form.maternity=recordinfo.maternitl
                            }else if(item=='familyill'){
                                that.form.familyall=recordinfo.familyill
                            }else if(item=='openrationrecord'){
                                that.form.operationrecord=recordinfo.openrationrecord
                            }else if(item=='bodymeax'){
                                that.form.bodyexam=recordinfo.bodymeax
                            }
                        }
                    }
                }else if(res.errcode==0&&(res.data!=null||res.data!='')){
                    that.ysrecordstate=1
                    localStorage.setItem('ysrecordstate',1)
                    that.fetchMegetMedicalLast()
                }else{
                    that.$Message.warning(res.errmsg);
                } 
            },
            async fetchMegetMedicalLast(){
                var that = this;
                var url = 'docMedRec/queryMegetMedicalLast';
                let patientinfo=JSON.parse(localStorage.getItem('userxinxi'));
                var card_no = patientinfo.cardNo;
                var data = {
                    card_no
            ...
            async unrecord(){
                let _this=this
                var data  =JSON.parse(localStorage.getItem('userxinxi'))
            ...
            init(){
                ...
                that.patientinname=data.name//患者姓名
                that.form.cardno=data.cardNo//患者卡号
                that.form.visitno=data.visitno
                that.getcurrentdayrecord()
            },
        },
        mounted(){
            let that=this
