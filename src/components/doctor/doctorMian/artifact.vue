    ...
    <div style="height:100%;overflow: auto;-webkit-overflow-scrolling: touch;" class="liao">
        ...
        <div class="content">
            ...
            <div class="message" v-for="(item,index) in comtent" :key='index'>
                <div class="receipt comment" v-if="item.identity!==identity" >
                    <p style="padding:5px 20px;font-size: 16px" v-if="item.identity=='1'">患者</p>
                    <p style="padding:5px 20px;font-size: 16px" v-else-if="item.identity=='2'">医师</p>
                    <p style="padding:5px 20px;font-size: 16px" v-else>助诊医师{{parseInt(item.identity) - 3}}</p>
                    <img src="@/common/imager/zhu.jpg" alt="" width="40px;" style="vertical-align: top;margin:0 15px;border-radius:50%;" />
...
import http from '@/utils/http';
    export default {
            ...
            var timerr = setInterval(async() => {
                if(localStorage.getItem('token')!=''&&localStorage.getItem('token')!=undefined&&localStorage.getItem('token')!=null){
                    ...
                    if(orderStart==2||orderStart==1){
                        var url = 'renew/genatareIdentity';
                        var data = {
                        ...
                        if(res.errcode==0){
                            ...
                            webSocket(that.orderNo,res.data.identity,that);
        ...
        methods: {
            ...
            gouserInfor(){
                var item = {}
                item.name=localStorage.getItem('userZixunNmme');
                item.cardNo=localStorage.getItem('cardeno_zixun');
                localStorage.setItem('userxinxi',JSON.stringify(item))
                 localStorage.setItem('tiao',4);
