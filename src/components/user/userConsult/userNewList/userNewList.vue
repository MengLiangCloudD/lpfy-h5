    ...
    <div style="height:100%;overflow: auto;-webkit-overflow-scrolling: touch;" class="MyExamine">
        ...
        <div class="content">
            ...
            <div class="mop"  v-for="(item,index) in huanList" :key="index" @click="addclick(item)">
                  <img src="@/common/imager/avatar.png" alt="" width="50px;" style="vertical-align: top;margin:0 15px;">
                  <div class="user">
                      <p style="color:#000;font-size: 16px;">{{item.doctor_name}}</p>
                      <p>{{item.created_at}}</p>
                  </div>
                  ...
                  <p class="poop" v-if="item.if_unread==1" style="background:red;width:10px;height:10px;border-radius: 50%;" ></p>
            </div>
        </div>
    ...
    export default {
        ...
        methods:{
            ...
            addclick(item){
                ...
                localStorage.setItem('docZixunNmme',item.name);
            },
            async getuserList(){
                var that = this;
                var  url = 'consultation/queryPatientNewsList';
