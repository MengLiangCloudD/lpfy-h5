<template>
    <div class="pull-refresh" ref="pullF" :style="{height: pullHeight}">
		<div class="pull-info" :style="{height: tipHeight}" v-if="!refreshing">
			<img src="./imager/refresh_arrow.png" :style="{transform:`rotate(${arrowDeg}deg)`,transition:transition}"/>
				<span>{{pullTip}}</span>
		</div>
		<div class="pull-info" :style="{height: tipHeight}" v-else>
			<img src="./imager/loading.gif"/>
...
export default {
    ...
    methods: {
      ...
      var el = document.getElementsByClassName('orderLists')[0];
			if(!this.disabled && moveY >= 0&&el.scrollTop==0) {
				e.preventDefault();
        this.moveY = moveY;
